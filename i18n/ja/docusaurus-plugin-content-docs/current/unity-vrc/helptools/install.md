...
VRChat Creator Companion がインストールされている場合は、以下のボタンをクリックしてリポジトリ（`https://vpm.logiccuteguy.com/index.json`）を追加し、ワンクリックでパッケージをインストールできます。VCC を使ったインストールのみがサポートされています。
